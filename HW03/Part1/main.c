...
if(rank ==1)
{
  ...
  printf("]\n");
int Alice = 0;
MPI_Send(message, Nmessages,MPI_UNSIGNED,Alice,11,MPI_COMM_WORLD);
MPI_Send(a,Nmessages,MPI_UNSIGNED,Alice,12,MPI_COMM_WORLD);
}
...
MPI_Status* status;
int otherPer= 1;
MPI_Recv(message,Nmessages,MPI_UNSIGNED,otherPer,11,MPI_COMM_WORLD,status);
MPI_Recv(a,Nmessages,MPI_UNSIGNED,otherPer,12,MPI_COMM_WORLD,status);
